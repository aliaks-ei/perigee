...
- Files: `thumbs/andromeda.webp`, `thumbs/betelgeuse.webp`, `thumbs/sirius.webp`, `thumbs/rigel.webp`, `encounters/the-galaxy-hiding-in-our-sky.jpg`
- Modifications: `thumbs/andromeda.webp` is generated by `scripts/andromeda-thumb.py`, which runs the same
  disc, spiral, dust-lane and edge-fade maths as
  `src/perigee/materials/GalaxyMaterial.ts` on the CPU and writes a 160×160 PPM.
  The three star thumbnails are generated by `scripts/star-thumbs.py` from the
  palettes and the cellular/marbled split in `materials/StellarMaterial.ts`,
  with the halo of `materials/GlareMaterial.ts` around the disc. None of these
  objects has a surface map to crop a thumbnail from, so the icons are drawn
  from the shaders instead and stay in step with what the renderer shows.
